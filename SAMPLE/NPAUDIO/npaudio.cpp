...
NPP_Destroy(NPP instance, NPSavedData** save)
{
    ...
    if (This != 0   )
    {
        ...
        // delete audio canvas and audio device
        if (This->audio)
        {
           delete This->audio;
           This->audio = 0;
...
NPP_SetWindow(NPP instance, NPWindow* window)
{
    ...
    if((window->window != 0   ) && (This->hWnd == 0   ))
    {
        ...
        This->win = IWindow::windowWithHandle(IWindowHandle(This->hWnd));
        if (This->win == 0)
        {
            This->win = new IWindow(IWindowHandle(This->hWnd));
            This->createdWin = TRUE;
        }
              ...
              uint16 *stype)
{
    ...
    // ask for the whole file (streaming data is in another sample)
    *stype = NP_ASFILE;
...
NPP_StreamAsFile(NPP instance, NPStream *stream, const char* fname)
{
   ...
   //Create the audio device, but if we get an exception then
   //we will still display the audio player, but it will be disabled
   try
   {
      This->wavePlayer = new IMMWaveAudio();
   }
   catch (...)
   {
      This->wavePlayer = 0;
   ...
   This->audio = new AUDIO(This->wavePlayer, This->win->size(), AUDIO_ID,
                           IString(fname), This->win, This->win);
   if (!This->wavePlayer)
   ;
//      ((IMultiCellCanvas*)This->audio)->disable();
   else
   {
       This->audio->sizeTo(This->win->size());
       This->audio->show();
       This->audio->setFocus();
       This->audio->refresh();
   }
    ...
    case WM_PAINT:
        {
            ...
            WinQueryWindowRect(This->hWnd, &invalidRect);
            WinFillRect(hps, &invalidRect, CLR_PALEGRAY);
            WinEndPaint(hps);
        return (MRESULT)0;
        }
        break;
