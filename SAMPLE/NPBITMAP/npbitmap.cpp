 ...
 *
 *
 *  This file uses the "shell" plugin as a basis for drawing a OS/2 bitmap
 *
 *  Changes from NPSHELL as marked with '##### change for NPBITMAP'
 *
 ***************************************************************************/
...
#include <IString.hpp>    // ##### change for NPBITMAP
#include <IGrafCtx.hpp>   // ##### change for NPBITMAP
#include <IGbitmap.hpp>   // ##### change for NPBITMAP
...
typedef struct _PluginInstance
{
    ...
    IGBitmap        *bitmap;             // ##### change for NPBITMAP
                ...
                NPSavedData* saved)
{
    ...
    This->pNext = 0   ;
    This->bitmap = 0;      // ##### change for NPBITMAP
...
NPP_Destroy(NPP instance, NPSavedData** save)
{
    ...
    // ##### change for NPBITMAP
    // Causes trap - is this a problem in OpenClass?
    // if (This->bitmap)
    //     delete This->bitmap;
...
NPP_StreamAsFile(NPP instance, NPStream *stream, const char* fname)
{
    ...
    // ##### change for NPBITMAP
    // create bitmap for use in drawing and printing
    This->bitmap = new IGBitmap(IString(fname));
...
void Draw(PluginInstance *This, HPS hps, POINTL *endPoint, BOOL fPrinting)
{
    ...
    {
        IGraphicContext gc = IGraphicContext(IPresSpaceHandle(hps));
        This->bitmap->sizeTo(ISize(endPoint->x, endPoint->y));
        This->bitmap->drawOn(gc);
    }
}
